...
int
ccsm_create_singleton_pid(int fd, const char *set_name,
		const char *singleton_name, pid_t pid)
{
	int retval;
	ccsm_ioctl_create_singleton_pid_t ioctl_args;

	if (strlen(set_name) >= CCSM_MAX_NAME_LEN ||
			strlen(singleton_name) >= CCSM_MAX_NAME_LEN) {
		return -EINVAL;
	}

	memset(&ioctl_args, 0, sizeof(ioctl_args));
	strcpy(ioctl_args.set_name, set_name);
	strcpy(ioctl_args.singleton_name, singleton_name);
	ioctl_args.pid = pid;

	retval = ioctl(fd, CCSM_IOCTL_CREATE_SINGLETON_PID, &ioctl_args);
	if (retval < 0) {
		retval = -errno;
	}

	if (retval) {
		fprintf(stderr, "ccsm_create_singleton_pid: set_name [%s] "\
				"pid [%d] singleton_name [%s] "\
				"error [%d:%s]\n",
				set_name, pid, singleton_name,
