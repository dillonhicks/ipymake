...
class InternalEvent(Entity):
	...
	def change_tag(self, new_tag):
		n = InternalEvent(self.cid, None, new_tag, self.extra_data)
		n.time = self.time
		return n

	def change_cid(self, new_cid):
		n = InternalEvent(new_cid, None, self.tag, self.extra_data)
		n.time = self.time
		return n
	...
	def change_extra_data(self, new_extra_data):
		n = InternalEvent(self.cid, None, self.tag, new_extra_data)
		n.time = self.time
		return n
