...
#define DRV_EXT		"-NAPI"
#endif
#define DRV_VERSION		"3.3.6"DRV_EXT
...
#ifdef CONFIG_KURT_CLOCK_SYNC
	/* obtain current clock cycle cnt & stuff it in the pkt */
	clksync_tx_cycles = get_cycles();
	transmit_time_sync_capture(skb);
#endif
...
#ifdef CONFIG_KURT_CLOCK_SYNC
		/* stuff the tsc for the last eth interrupt into this pkt */
		receive_time_sync_capture(skb);
#endif		
...
#ifdef CONFIG_KURT_CLOCK_SYNC
	/* register a eth device */
	if (clksync_register_eth(netdev->name, pdev->irq)) {
		printk(KERN_ERR "clksync: failed to register %s\n", netdev->name);
	}
#endif
