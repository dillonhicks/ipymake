...
#define WORKLOOP_ITERS 1592489
static float dummy = 2.7;
static inline void workloop(void)
{
...
static void *node_code(void *arg)
{
	...
	if (use_gs) {
		/* if source */
		if (n->read_fd == -1)
			grp_task_join_group(grp_fd, n->pipeline->bp_grp_name, gettid(), n->name);
		else
			grp_task_join_group(grp_fd, n->pipeline->seq_grp_name, gettid(), n->name);
